...
class DES extends Feistel {
   ...
   /*
    * copy the output of your code for P3 here, that is, the declaration of
    * int[][] ks = { ... };
    */
   static int[][] ks = {
      {},
   ...
   static int[][] getSubKeys(String keyHex) {
      ...
      var key = Utils.hexToBinString(keyHex, 64);
      int i = 0;
      for (int[] k : ks) {
         if (k.length == 0) {
            continue;
         }
         subkeys[++i] = Utils.applyPermut(
                 k,
               Utils.binStringToIntArray(key));
      }
      return subkeys;
   }// getSubKeys method
   ...
   int[] encryptDES(int[] block) {
      return Utils.applyPermut(IPinv, encrypt(Utils.applyPermut(IP, block)));
   }// encryptDES method
   ...
   int[] decryptDES(int[] block) {
      return Utils.applyPermut(IPinv, decrypt(Utils.applyPermut(IP, block)));
   }// decryptDES method
}// DES class
...
class DESround extends FeistelFunction {
   ...
   int[] runThroughSboxes(int[] data) {
      ...
      for (int l = 0; l < data.length - 5; l += 6) {
         int sBoxNum = (l / 6) + 1;
         int row = (data[l] << 1) | data[l + 5];
         int col = data[l + 1] << 3 | 
                   data[l + 2] << 2 | 
                   data[l + 3] << 1 | 
                   data[l + 4];
         var value = S[sBoxNum][row][col];
         int pos = (sBoxNum - 1) * 4;
         output[pos] = value >> 3 & 1;
         output[pos + 1] = value >> 2 & 1;
         output[pos + 2] = value >> 1 & 1;
         output[pos + 3] = value & 1;
      }
      return output;
   ...
   int[] round(int[] data, int[] subkey) {
      var value = Utils.applyPermut(P,
            runThroughSboxes(Utils.XOR(Utils.applyPermut(expandP, data),
                  subkey)));
      return value;
   }// round method
